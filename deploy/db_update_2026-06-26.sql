-- =====================================================================
-- テストサーバー用 DB 更新ファイル（2026-06-26）
-- =====================================================================
-- このファイル1つを phpMyAdmin の「SQL」タブに貼り付けて実行すれば、
-- 決済機能に必要な DB の準備がすべて整います。
--
-- やっていること:
--   ① 足りないテーブルを作る（既にあるテーブルはそのまま・壊しません）
--   ② result_access_tokens.expires_at を「空っぽ(NULL)OK」に変更
--      （500円プラン購入者の無期限トークンを保存できるようにする）
--
-- 何回実行しても安全です（既にある物は作り直さない／同じ設定を上書きするだけ）。
-- =====================================================================


-- ---------------------------------------------------------------------
-- ① 足りないテーブルを作る（IF NOT EXISTS = 無ければ作る、あれば触らない）
-- ---------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS diagnosis_sessions (
    ...
    has_children BOOLEAN COMMENT '子供の有無',
...
-- ---------------------------------------------------------------------
-- ② result_access_tokens.expires_at を「空っぽ(NULL)OK」に変更
--    （古いDBで NOT NULL のままでも、ここで NULL 許容へ直す）
--    既に NULL 許容なら同じ設定を上書きするだけなので無害。
-- ---------------------------------------------------------------------
ALTER TABLE result_access_tokens
    MODIFY COLUMN expires_at TIMESTAMP NULL
    COMMENT '有効期限（NULL=無期限。500円プラン購入者は無期限）';


-- =====================================================================
-- 確認用（任意）: 実行後にこの2つを流すと結果が見られます
--   SHOW TABLES;                                       -- 6個そろっているか
--   SHOW COLUMNS FROM result_access_tokens LIKE 'expires_at';  -- Null が YES か
-- =====================================================================
